...
export default function BoardDetail() {
  ...
  useEffect(() => {
    ...
    // type이 'deal'이면 모든 거래 타입을 시도
    // type이 'person'이면 모든 인원 모집 타입을 시도
    const tryFetchBoard = async () => {
      ...
      for (const boardType of boardTypes) {
        try {
          console.log(`시도 중: /board/${boardType}/${board_id}`);
          
          const response = await axios.get(`/board/${boardType}/${board_id}`);
          console.log('성공한 응답:', response.data);
          setBoard(response.data);
          return; // 성공하면 종료
        } catch (err) {
          console.log(`${boardType} 타입으로 조회 실패, 다음 타입 시도`);
          continue; // 실패하면 다음 타입 시도
        }
      }
